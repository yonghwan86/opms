...
function MetricCard({
  ...
  title: string; subtitle?: React.ReactNode; source?: string; live?: boolean; headerRight?: React.ReactNode; icon: React.ElementType; iconBg: string; loading?: boolean; children: React.ReactNode;
}) {
  return (
        ...
        <Card className="border border-border bg-card">
            ...
            <div className="flex flex-col gap-2 pb-2 border-b border-border">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                {oilAnalysisTab === 'comparison' && (
                  <div className="flex gap-1 flex-shrink-0">
                    {([
                      { id: 'gasoline', label: '휘발유', active: 'bg-yellow-100 text-yellow-700 border-yellow-400' },
                      { id: 'diesel', label: '경유', active: 'bg-green-100 text-green-700 border-green-400' },
                      { id: 'kerosene', label: '등유', active: 'bg-sky-100 text-sky-700 border-sky-400' },
                    ] as const).map(f => (
                      <button key={f.id}
          ...
          <div className="px-2 pb-2 pt-1">
            {oilAnalysisTab === 'global' ? (
              ...
              const allVals = compRows.flatMap(r => [r.intlKrw, r.pretax]).filter((v): v is number => v !== null);
              ...
              return (
                <div>
                  <ResponsiveContainer width="100%" height={310}>
                    <ComposedChart data={compRows} margin={{ top: 30, right: 8, left: 10, bottom: 20 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false} />
                      <XAxis dataKey="label"
                        tick={{ fontSize: 11, fill: "#111827", fontWeight: 700, textAnchor: "middle" }}
